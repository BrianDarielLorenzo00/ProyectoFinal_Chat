...
firebase.auth().onAuthStateChanged( user => {
    if(user){
        ...
        contenidoProtegido.innerHTML = /*html*/`
            <p class=" Y text-center lead mt-5">Debes de iniciar Sesion</p>
        `
        ...
        .onSnapshot(query => {
            ...
            query.forEach(doc => {
                ...
                if(doc.data().uid === user.uid){
                    ...
                    <p class="r text-center lead mt-2 DevCode">${doc.data().texto}</p>
                    </div>
                 `
                }else{
                    contenidoProtegido.innerHTML += `
                    <div class="d-flex justify-content-start">
                    <p class="p text-center lead mt-2">${doc.data().texto}</p>
                    </div>
                 `
                }
